...
BEGIN_MESSAGE_MAP(CRegDialog, CDialogEx)
	...
	ON_STN_CLICKED(IDC_STATIC_REGMETHODINFO, &CRegDialog::OnStnClickedStaticRegmethodinfo)
END_MESSAGE_MAP()
...
void CRegDialog::OnBnClickedButtonRegok()
{
	...
	if (strInput == L"JXLEYO-HRP-MLTP-DRVR") {//验证注册码输入//SN注册序列号字符串,实际软件注册验证不检测内容，只有注册文件存在即表示成功
		...
		AfxMessageBox(L"Administrator permission is required to register the software. Click OK and select allow to change the computer!", MB_OK, MB_ICONINFORMATION);//注册软件需要管理员权限，点击确定后请选择允许更改计算机
		...
		AfxMessageBox(L"Serial number error, please re-enter!", MB_OK, MB_ICONSTOP);//序列号错误，请重新输入!
	}
...
void CRegDialog::OnStnClickedStaticRegmethodinfo()
{
	// TODO: 在此添加控件通知处理程序代码
}
