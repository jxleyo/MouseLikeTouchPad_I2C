...
BEGIN_MESSAGE_MAP(CMltpSvcDlg, CDialogEx)
	...
	ON_STN_CLICKED(IDC_STATIC_COPYRIGHT, &CMltpSvcDlg::OnStnClickedStaticCopyright)
	ON_BN_CLICKED(IDC_BUTTON_REG, &CMltpSvcDlg::OnBnClickedButtonReg)
END_MESSAGE_MAP()
...
void CMltpSvcDlg::AddTrayIcon()
{
	...
	wcscpy_s(m_nid.szTip, L"MouseLikeTouchPad Service Guide");                // 信息提示条////仿鼠标触摸板服务指南//MouseLikeTouchPad Service Guide
...
LRESULT CMltpSvcDlg::OnSystemTray(WPARAM wParam, LPARAM lParam)
{
...

	{
		...
		case WM_RBUTTONUP: {    // 右键起来时弹出菜单
			...
			PopMenu.AppendMenu(MF_STRING, ID_MANUAL, L"Manual");//说明手册
			PopMenu.AppendMenu(MF_STRING, ID_ABOUT, L"ServiceGuide...");//服务指南
			PopMenu.AppendMenu(MF_STRING, IDM_EXIT, L"Close");//PopMenu.AppendMenu(MF_STRING, WM_DESTROY, L"关闭");//关闭
...
void CMltpSvcDlg::ShowImage()
{
	...
	if (!LoadImageFromRes(&Image, IDB_JPG_SKETCHEN, _T("JPG"))) return;//IDB_JPG_SKETCHCN//
	if (Image.IsNull())
	{
		MessageBox(_T("IMAGE Res Load Err"));
		//return -1;
...
void CMltpSvcDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == 3) {//定时检测到期时间事件
		CheckExpirationTime();//再次调用检测
	}
...
void CMltpSvcDlg::CheckRegStatus()
{
	...
	if (IsRegistered) {
		...
		m_Static_RegInfo.SetWindowText(L"Registered");//Registered//软件已注册(永久使用权)
	}
	else {
		m_Button_Reg.EnableWindow(TRUE);
		m_Static_RegInfo.SetWindowText(L"UnRegistered");	//UnRegistered//软件未注册(30天使用期)
	}
}


void CMltpSvcDlg::CheckExpirationTime()
{
	if (IsRegistered) {
		ExpirationTimeChecked = TRUE;//已注册无需检测到期时间
		return;
	}

	if (!LogFileExist(L"Installed.dat")) {
		AfxMessageBox(L"Your software is not installed correctly. Click OK and select allow to change the computer to complete the installation!", MB_OK, MB_ICONSTOP);////您的软件未正确安装，点击确定后请选择允许更改计算机完成安装！
		...
		//获取文件属性
		CFileStatus fstatus;
		if (CFile::GetStatus(szPath, fstatus)) {
			ExpirationTimeChecked = TRUE;
			KillTimer(3);//终止定时检测

			//创建时间
			CTime instTime = fstatus.m_ctime;
			...
			if (interval > 30) {
				AfxMessageBox(L"Your trial period has expired. Please purchase the software and register!", MB_OK, MB_ICONSTOP);//您的试用期已经超过，请购买软件并注册！
			}
		}
		else {
			//AfxMessageBox(L"检测注册失败！", MB_OK, MB_ICONSTOP);
			//检测失败，再次定时间隔检测
		    SetTimer(3, 10000, NULL);//计时器3，10秒触发一次OnTimer()函数
		}
	}
...
void CMltpSvcDlg::OnStnClickedStaticCopyright()
{
	// TODO: 在此添加控件通知处理程序代码
}


void CMltpSvcDlg::OnBnClickedButtonReg()
{
	// TODO: 在此添加控件通知处理程序代码
}
