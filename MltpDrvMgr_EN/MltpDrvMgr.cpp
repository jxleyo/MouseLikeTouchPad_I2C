...
static wchar_t TouchPad_I2C_devInstanceID[MAX_DEVICE_ID_LEN];
                     ...
                     _In_ int       nCmdShow)
{
    ...
    if (verInfo.dwMajorVersion < 10) {
        MessageBox(NULL, L"System versions other than windows10/11 x64 are not supported!", L"MltpDrvMgr", MB_OK);//不支持Windows10/11 x64以外的系统版本
        return EXIT_FAIL;
    }

    if (verInfo.dwBuildNumber < 19041) {//Win10 Build 19041.208(v2004)正式版
        MessageBox(NULL, L"The Windows version is lower than the official version of win10 build 19041.208 (V2004). Please upgrade the system and try again.", L"MltpDrvMgr", MB_OK);//Windows版本低于Win10 Build 19041.208(v2004)正式版，请升级系统后再试。
        return EXIT_FAIL;
    }
    ...
    if (!MainFileExist(L"Uninst.bat")) {
        MessageBox(NULL, L"The file Uninst.bat is missing. Please download the installation package again", L"MltpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }

    if (!DrvFileExist(L"MouseLikeTouchPad_I2C.inf")) {
        MessageBox(NULL, L"The file MouseLikeTouchPad_I2C.inf is missing. Please download the installation package again", L"MltpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }
    if (!DrvFileExist(L"MouseLikeTouchPad_I2C.cat")) {
        MessageBox(NULL, L"The file MouseLikeTouchPad_I2C.cat is missing. Please download the installation package again", L"MltpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }
    if (!DrvFileExist(L"MouseLikeTouchPad_I2C.sys")) {
        MessageBox(NULL, L"The file MouseLikeTouchPad_I2C.sys is missing. Please download the installation package again", L"MltpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }
...
BOOL FindDevice()
{
    ...
    TCHAR devInstanceID[MAX_DEVICE_ID_LEN];//instance ID
    LPTSTR* devHwIDs;
    ...
    if (!bTouchPad_I2C_FOUND) {
        printf("I2C device of touchpad not found!\n");//未找到TouchPad触控板的I2C设备！
    }
    else {
...
BOOL SaveTouchPad_I2C_devInstanceID()
{
    ...
    wcscpy_s(szFilePath, exeFilePath);
    wcscat_s(szFilePath, L"\\LogFile\\");
    wcscat_s(szFilePath, L"TouchPad_I2C_devInstanceID.txt");
    ...
    //返回接受字符串所需缓冲区的大小，已经包含字符结尾符'\0'
    iSize = WideCharToMultiByte(CP_ACP, 0, TouchPad_I2C_devInstanceID, -1, NULL, 0, NULL, NULL); //iSize =wcslen(TouchPad_I2C_hwID)+1
    pszMultiByte = (char*)malloc(iSize * sizeof(char)); //不需要 pszMultiByte = (char*)malloc(iSize*sizeof(char)+1);
    WideCharToMultiByte(CP_ACP, 0, TouchPad_I2C_devInstanceID, -1, pszMultiByte, iSize, NULL, NULL);
...
FindDev:
    ...
    if (!LogFileExist(L"TouchPad_FOUND.txt")) {
        MessageBox(NULL, L"No matching touchpad device found, unable to install driver.", L"MltpDrvMgr", MB_OK);
        ExitCode = EXIT_FAIL;
        goto Clean;
    }
    ...
    if (!LogFileExist(L"TouchPad_I2C_FOUND.txt")) {
        MessageBox(NULL, L"No matching I2C bus touchpad device was found, and the driver cannot be installed.", L"MltpDrvMgr", MB_OK);
        ExitCode = EXIT_FAIL;
        goto Clean;
    }
...
CheckDev:
    ...
    Sleep(1500);
    if (retry > 3) {
        MessageBox(NULL, L"Failed to find the device after secondary verification! Please try again later.", L"MltpDrvMgr", MB_OK);
        ExitCode = EXIT_FAIL;
        goto Clean;
    }
    ...
    if (!LogFileExist(L"OEMDriverName.txt")) {
        MessageBox(NULL, L"The second verification failed to install the driver. Please try again later.", L"MltpDrvMgr", MB_OK);
        ExitCode = EXIT_FAIL;
        goto Clean;
    }
...
InstSuccess:
    ...
    //存在驱动
    MessageBox(NULL, L"Driver installed successfully! Please check whether the touch panel works normally. If there is no response, please restart the computer to make the driver take effect!", L"MltpDrvMgr", MB_OK | MB_DEFBUTTON1);
    ExitCode = EXIT_OK;
...
FindDev:
    ...
    Sleep(1500);
    if (retry > nTry) {
        MessageBox(NULL, L"Failed to find the device! Cancel uninstall, please try again later.", L"MltpDrvMgr", MB_OK);
        ExitCode = EXIT_FAIL;
        return;
    }
...
CheckDev:
    nRet = WinExec("MltpDrvMgr.exe FindDevice", SW_HIDE);
    if (nRet < 31) {
        MessageBox(NULL, L"Winexec failed to call finddevice subroutine after secondary verification! Cancel uninstall, please try again later.", L"MltpDrvMgr", MB_OK);
        ExitCode = EXIT_FAIL;
        return;
    }
...
UninstDrvSuccess:
    ...
    //不存在驱动
    ExitCode = EXIT_OK;
    MessageBox(NULL, L"Unload the driver successfully!", L"MltpDrvMgr", MB_OK | MB_DEFBUTTON1);
...
void InstallShortcut()
{
    ...
    if (GetDesktopPath(szDesktopPath)) {
        ...
        if (!CreateShotCut(szSourcePath, L"MltpSvc.exe", L"ShowDialog", L"MouseLikeTouchPad_ServiceGuide", szDesktopPath)) {//仿鼠标触摸板服务指南//MouseLikeTouchPad_ServiceGuide
    ...
    if (GetProgramsPath(szProgramsPath)) {
        ...
        if (CreateDirectory(strDestDir, NULL)) {//程序组文件夹必须存在才能建立快捷方式
            CreateShotCut(szSourcePath, L"MltpSvc.exe", L"ShowDialog", L"MouseLikeTouchPad_ServiceGuide", strDestDir);//仿鼠标触摸板服务指南//MouseLikeTouchPad_ServiceGuide
            //MessageBox(NULL, szSourcePath, L"CreateShotCut szProgramsPath err", MB_OK);

            CreateShotCut(szSourcePath, L"MltpDrvMgr.exe", L"Uninstall", L"MouseLikeTouchPad Uninstaller", strDestDir);//仿鼠标触摸板卸载程序//MouseLikeTouchPad Uninstaller
    ...
    wcscpy_s(szSourcePath, exeFilePath);//不含/
    wcscat_s(szSourcePath, L"\\MltpSvc.exe");
    WriteSzReg(L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", L"MouseLikeTouchPad_ServiceGuide", szSourcePath);

    //创建注册表程序卸载项
    TCHAR strUninstDir[MAX_PATH];
    wcscpy_s(strUninstDir, exeFilePath);
    wcscat_s(strUninstDir, L"\\MltpDrvMgr.exe Uninstall");
    WriteSzReg(L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\MouseLikeTouchPad", L"DisplayName", L"MouseLikeTouchPad Uninstaller");//MouseLikeTouchPad Uninstaller
...
void UninstallShortcut()
{
    ...
    TCHAR szDesktopPath[MAX_PATH];
    if (GetDesktopPath(szDesktopPath)) {
        wcscat_s(szDesktopPath, L"\\MouseLikeTouchPad_ServiceGuide.lnk");//仿鼠标触摸板服务指南//MouseLikeTouchPad_ServiceGuide
    ...
    //删除开机启动项
    DelRegkey(L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", L"MouseLikeTouchPad_ServiceGuide");//仿鼠标触摸板服务指南//MouseLikeTouchPad Service Guide
...
{
    //删除开机启动项
    if (DelRegkey(L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", L"MouseLikeTouchPad_ServiceGuide")) {//仿鼠标触摸板服务指南//MouseLikeTouchPad_ServiceGuide
        MessageBox(NULL, L"Software startup item deleted successfully!", L"MltpDrvMgr", MB_OK);
    }
    else {
        MessageBox(NULL, L"Software startup item deletion failed!", L"MltpDrvMgr", MB_ICONSTOP);
    }
}
...
void Register()
{
    ...
    if (LogFileExist(L"RegKey.dat")) {
        ...
        if (!NewLogFile(L"RegKey.dat")) {
            MessageBox(NULL, L"Software registration failed, please try again!", L"MltpDrvMgr", MB_ICONSTOP);
        }
        else {
            isRegistered = TRUE;
        }
    ...
    if (isRegistered) {
        //终止和重启服务程序运行
        WinExec("taskkill /f /im MltpSvc.exe", SW_HIDE);
        MessageBox(NULL, L"Software registration succeeded!", L"MltpDrvMgr", MB_OK);

        //重启服务程序运行
        WinExec("MltpSvc.exe SHowDialog", SW_SHOW);

        int result = MessageBox(NULL,L"After skillfully using the touch pad driver, you don't need to start the help service program. Are you sure to delete the start item?", L"MltpSvc", MB_YESNO);
        switch (result)
        {
            case IDYES:
            {
